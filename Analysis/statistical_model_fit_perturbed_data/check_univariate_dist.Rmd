...
get_Rimage_data<-function(all_ud_i){
    ...
    # Statistics on seasonal phase
    output$stat_store = random_env$stat_store
...
# Check that the event_statistics is unique in every session [i.e. we didn't
# have problems with random number generation]. We perturbed hsig, duration,
...
# Check how the 'optimal seasonal phase' varies due to jittering
# Units are in years
best_phase_difference = sapply(store_var_list, 
    f<-function(x) x$stat_store$phi - original_var_list$stat_store$phi)
best_phase_difference = as.data.frame(t(best_phase_difference))
names(best_phase_difference) = original_var_list$stat_store[,1]
# Note very small but systematic difference for Hsig and Tp1 (0.02 years ~ 1
# week)
print(summary(best_phase_difference))
...
# Look at the automatically chosen copula family. Mostly stable to perturbations,
