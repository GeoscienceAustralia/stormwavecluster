...
#
get_Rimage_data_vine_copula<-function(all_ud_i){
    library(evmix)
    library(logspline)
    library(VineCopula)
    ...
    #
    #
    # The variables below are 'the same' as those we collected from
    # the univariate_distributions sessions -- as it might be useful, and 
    # they are retained in the session
    #
    #
    ...
    #
    #
    # The variables below are specific to the copula modelling
    #
    #

    output$events_conditional_copuladata = random_env$events_conditional_copuladata
    output$conditional_variables = random_env$conditional_variables
    output$es_cop = random_env$es_cop

    output$c_vine_node_order = random_env$c_vine_node_order
    output$es_cop_reorder = random_env$es_cop_reorder

    output$copula_model = random_env$copula_model
    output$twocopula_test = random_env$twocopula_test

    output$copula_model2 = random_env$copula_model2
    output$twocopula_testB = random_env$twocopula_testB
