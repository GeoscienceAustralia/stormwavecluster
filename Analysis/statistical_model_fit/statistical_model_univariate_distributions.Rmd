...
tideResid_mixture_fit = evmix_fit$fit_gpd_mixture(
    ...
    bulk='normal',
    #
    # Here we reduce the range of thresholds which are tested as initial
    # guesses for optimization.
    # This helps prevent occasional fits with thresholds > upper_limit on
    # randomized data. 
    # Note we are not imposing a 'hard' limit -- just restricting the initial parameter
    # space of thresolds that we search. 
    # If the subsequent Baysian fit finds a better value, it will update the ML
    # estimate in tideResid_mixture_fit [but the Bayesian fit also
    # respects the constraints].
    #
    # See 'evmix_fit$constrained_fit_gpd_mixture' for a stronger approach to constrained ML,
    # if needed.
    #
    gpd_threshold_quantile_range=c(0.05, 0.8) 
    )
